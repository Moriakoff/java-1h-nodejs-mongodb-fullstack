...
app.post('/students/save', function (request, response) {
    ...
    mongo.connect(dbURL, function (err, databaseConnection) {
        if (err) {
            throw err;
        }
        ...
        databaseObject.collection("students").insertOne(student, function (err, res) {
            ...
            response.redirect("/"); //firstName, lastName, receivedDate
...
app.get('/students-add', function (req, res) {
    res.sendFile(__dirname + '/content/studentAdd.html');
});

app.get('/profiles/:studentId', function (req, res,) {
    res.sendFile(__dirname + "/content/profile.html",);
});
